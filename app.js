...
app.post('/showSal/addSal', function(req, res, next) {
	...
	var recieved_date = req.body.recieve_date;
	   ...
	   ('${emp_id}', (Select grade_id from pay_grade where grade_name='${grade_name}'),'${emp_salary_month}','${emp_salary_year}','${recieved_date}',(Select grade_basic+grade_da+grade_bonus-grade_pf from pay_grade where grade_name='${grade_name}'),(Select (grade_basic+grade_da+grade_bonus-grade_pf)*12 from pay_grade where grade_name='${grade_name}') )`;
   	connection.query(sql,function (err, data) {
	  if (err){
		  console.log(err)
